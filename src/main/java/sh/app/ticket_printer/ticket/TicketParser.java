...
public class TicketParser {
	...
	static {
		...
		fontFamilies = new HashSet<String>(Arrays.asList(ge
				.getAvailableFontFamilyNames()));
	}

	public static Ticket parse(String xml)
			throws IncorrectTicketFormatException, TicketParserException {
		if (isLogEnabled()) {
			System.out
					.println("DEBUG: TicketParser.start(): got xml to parse: "
							+ xml);
		}
	...
	private static synchronized void validate(byte[] xmlBytes)
		...
		try {
			...
			throw new IncorrectTicketFormatException(
					"Некорректный формат описания билета", e);
		}
	}

	private static void processFormElement(StartElement element,
			Ticket targetTicket) {
		Form form = new Form();
		form.setHeight(Integer.valueOf(element.getAttributeByName(QNAME_HEIGHT)
				.getValue()));
		form.setWidth(Integer.valueOf(element.getAttributeByName(QNAME_WIDTH)
				.getValue()));

		targetTicket.setForm(form);
	}

	private static void processTextElement(XMLEventReader reader,
	...
	private static void processAbstractTicketAttr(
		...
		if (rotationAttr != null) {
			...
			try {
				...
				throw new CustomTicketParserException(
						"Incorrect value for the element's attribute 'rotation' found: " + rotationAttr.getValue());
			}
	...
	private static String parseText(XMLEventReader reader)
			throws CustomTicketParserException, XMLStreamException {
		if (!reader.hasNext()) {
